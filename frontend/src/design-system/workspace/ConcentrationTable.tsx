          ...
          <table className="w-full border-collapse text-xs">
            <thead>
              <tr className="border-b border-v2-border-sub">
                <th className="text-left text-[10px] uppercase tracking-widest text-v2-text-3 font-semibold pl-5 py-2">
                  {isEn ? 'Symbol' : '标的'}
                </th>
                <th className="text-left text-[10px] uppercase tracking-widest text-v2-text-3 font-semibold py-2">
                  {isEn ? 'Strategy' : '策略'}
                </th>
                <th className="text-right text-[10px] uppercase tracking-widest text-v2-text-3 font-semibold py-2">
                  {isEn ? 'Delta Exp' : 'Delta敞口'}
                </th>
                <th className="text-right text-[10px] uppercase tracking-widest text-v2-text-3 font-semibold py-2">
                  {isEn ? 'Theta/day' : 'Theta/日'}
                </th>
                <th className="text-right text-[10px] uppercase tracking-widest text-v2-text-3 font-semibold pr-5 py-2">
                  {isEn ? 'Margin' : '保证金'}
                </th>
              </tr>
            </thead>
            <tbody>
              {rows.map(({ symbol, delta, theta, margin, strategies }) => (
                <tr key={symbol} className="border-b border-v2-border-sub hover:bg-v2-surface-hover transition-colors">
                  <td className="pl-5 py-2.5 font-bold text-v2-text-1">{symbol}</td>
                  <td className="py-2.5">
                    <div className="flex flex-wrap gap-1">
                      {strategies.length > 0 ? strategies.map((s) => (
                        <span key={s} className="text-[10px] px-1.5 py-0.5 rounded-md bg-v2-surface-alt text-v2-text-3 font-semibold">
                          {s}
                        </span>
                      )) : (
                        <span className="text-[10px] text-v2-text-3">STOCK</span>
                      )}
                    </div>
                  </td>
                  <td className={`py-2.5 text-right tnum font-semibold ${delta >= 0 ? 'text-v2-positive' : 'text-v2-negative'}`}>
                    {delta >= 0 ? '+' : ''}{fmtNum(String(delta.toFixed(2)))}
                  </td>
                  <td className={`py-2.5 text-right tnum font-semibold ${theta >= 0 ? 'text-v2-positive' : 'text-v2-negative'}`}>
